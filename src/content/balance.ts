...
export const BALANCE = {
  ...
  // ---- Waking scaling per snuff ----
  ng: {
    hpGrowth: 1.45,
    dmgGrowth: 1.2,
    soulGrowth: 1.55, // marrow outgrow HP: a cycle is net-positive, so the 20 minutes after a Snuff are faster than the 20 before
    dropGrowth: 1.35,
  },
  ...
  player: {
    ...
    hpPerLevel: 1.035, // multiplicative HP per level ("marrow hardening")
    dmgPerLevel: 1.025, // multiplicative damage per level: levels always pay, stats decide how
    staminaBase: 60,
    staminaPerEnd: 1.6,
    staminaRegenBase: 18,
    ...
    critMult: 2.0,
    baseCrit: 0.05,
    critPerDex: 0.0025, // per fin point up to 40
    riposteWindow: 2.0,
    /** heavy strain reduces enemy attack progress by this fraction on strain */
  ...
  level: {
    ...
    startingStats: { vit: 10, bre: 10, mig: 10, fin: 10, ins: 8, dev: 8 },
  },
  ...
  weapon: {
    ...
    reqPenalty: 0.5,
    /** infusion: base multiplier and scaling override might */
    infusion: {
      heavy: { base: 0.95, grade: 'A', stat: 'mig' },
      keen: { base: 0.95, grade: 'A', stat: 'fin' },
      magic: { base: 0.9, grade: 'A', stat: 'ins', type: 'magic' },
      blessed: { base: 0.9, grade: 'A', stat: 'dev', type: 'lightning' },
  ...
  shade: {
    ...
    hpPerLevel: 1.06,
    xpPerKillMult: 1.0,
    /** fraction of Marrow value that shade kills yield when hunting */
    huntSoulFrac: 0.45,
    /** fraction of drops when hunting */
    huntDropFrac: 0.6,
    /** beside-the-player damage fraction vs hunting damage */
    besideDmgMult: 1.0,
    /** cortege retreats for this long after a wipe */
    retreatTime: 30,
    /** cortege recovery per second as a fraction of its max hp (before healers) */
    baseRegenFrac: 0.05,
    /** below this uptime (recovery / incoming) the cortege wipes and retreats */
    wipeUptime: 0.25,
  },
  ...
  prestige: {
    ...
    minHumanity: 1,
    /** Dark Severing unlock Waking count */
    severingAt: 5,
    threadDivisor: 25,
    threadExponent: 0.6,
    ageOfDarkAt: 3, // severings required
  },

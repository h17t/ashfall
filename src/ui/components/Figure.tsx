 ...
 * silhouette mask: blood for the hit frame, mote for the open reprisal, verdigris for poison,
 * wisp-blue for frost. One tint at a time; the loudest wins.
 */
interface Props { kind: 'enemy' | 'boss'; id: string; hurt: boolean; broken: boolean; poison: boolean; frost: boolean; bleed: number; big: boolean }

export const Figure = memo(function Figure({ kind, id, hurt, broken, poison, frost, bleed, big }: Props) {
  if (!hasAsset(kind, id)) {
    const shape = kind === 'boss' ? getBoss(id).shape : getEnemy(id).shape;
    return <div className="h-[70%]"><EnemySprite shape={shape} phase={0} broken={broken} hurt={hurt} /></div>;
  }
  const tint = hurt
    ...
    : broken
      ...
      : frost
        ? { color: 'var(--wisp)', opacity: 0.45, blend: 'screen' as const }
        : poison

...
export const Bonfire = memo(function Bonfire() {
  ...
  const hue = dark > 0 ? 270 : sigils > 0 ? 300 : 22;
  return (
    <div className="pointer-events-none fixed bottom-0 left-1/2 -translate-x-1/2 z-0 flex items-end justify-center" style={{ width: 240 * size, height: 160 * size, opacity: 0.9 }} aria-hidden>
      <svg viewBox="0 0 120 100" className="w-full h-full">
        <defs>
          <radialGradient id="glow" cx="50%" cy="80%" r="60%"><stop offset="0%" stopColor={`hsl(${hue} 90% 60% / 0.55)`} /><stop offset="100%" stopColor="transparent" /></radialGradient>
        </defs>
        <ellipse cx="60" cy="88" rx="60" ry="30" fill="url(#glow)" />
        <g className="flame" style={{ transformOrigin: '60px 90px' }}>
          <path d="M60 90 C40 75 42 55 52 45 C50 60 58 62 58 50 C64 58 70 60 66 42 C80 55 78 78 60 90 Z" fill={`hsl(${hue} 95% 55%)`} opacity="0.9" />
          <path d="M60 90 C50 80 50 66 56 58 C56 66 60 68 60 60 C64 66 66 70 64 58 C72 68 70 82 60 90 Z" fill={`hsl(${hue + 20} 100% 75%)`} opacity="0.9" />
        </g>
        <path d="M30 90 L90 90 L82 96 L38 96 Z" fill="#2a2626" />
        <path d="M45 90 L60 82 L75 90 Z" fill="#1d1a1a" />
      </svg>
    </div>
  );
});

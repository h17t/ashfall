...
/** Recitation slots + FP. Hotkeys 1–6 cast. Hidden until the player owns a catalyst/spell. */
export const SpellBar = memo(function SpellBar() {
  ...
  return (
    <Slab material="iron" seed="spells" ornament="none" className="px-4 py-2.5 flex items-center gap-4">
      <div className="w-32"><Gauge value={fp} max={fpMax} tone="wisp" label="Focus" text={`${fp} / ${fpMax}`} height={6} /></div>
      <div className="flex gap-2 flex-1">
        {Array.from({ length: slots }, (_, i) => <Slot key={i} slot={i} known={known.split(',').filter(Boolean)} />)}
        {slots === 0 && <span className="t-lore text-[13px]">You know a spell but have no recitation slot. A catalyst or a Snuffed gift will open one.</span>}
      </div>
    </Slab>
  );
});
...
function Slot({ slot, known }: { slot: number; known: string[] }) {
  ...
  const id = useSel((s) => s.player.recited[slot] ?? null);
  ...
  return (
    <div className="relative">
      <Tooltip tip={def ? <div><div className="font-display text-[16px]">{def.name}</div><div className="italic">{def.lore}</div><div className="font-num mt-1">{def.school} · {def.fp} FP · {def.cooldown}s cooldown · key {slot + 1}</div></div> : 'Empty recitation slot. Click to recite a known spell.'}>
        <button
        ...
        >
          ...
          <span style={{ color: 'color-mix(in srgb, var(--bone) 72%, transparent)' }}>{slot + 1}</span>{def ? def.name : 'recite'}{cd > 0 && <span className="t-num ml-1" style={{ color: 'var(--bone)' }}>{cd.toFixed(0)}s</span>}
        </button>
      </Tooltip>
      {pick && (
        <div className="absolute z-40 top-full mt-1 slab p-2 flex flex-col gap-1 min-w-[160px]">
          {known.map((k) => <button key={k} className="btn" onClick={() => { dispatch({ type: 'recite', slot, spell: k }); setPick(false); }}>{getSpell(k).name}</button>)}
          {def && <button className="btn" onClick={() => { dispatch({ type: 'recite', slot, spell: null }); setPick(false); }}>Unrecite</button>}

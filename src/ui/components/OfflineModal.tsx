...
export function OfflineModal() {
  ...
  const off = useSel((s) => (s.offline ? JSON.stringify({ ...s.offline, marrow: s.offline.marrow.toString(), kills: s.offline.kills.toString(), phantomXp: s.offline.phantomXp.toString() }) : ''));
  if (!off) return null;
  const o = JSON.parse(off) as { seconds: number; cappedSeconds: number; marrow: string; materials: Record<string, number>; kills: string; phantomXp: string; zone: string; tier: number; wiped: boolean };
      ...
      <Slab material="stone" seed="away" rough={7} className="p-6 max-w-md w-full flex flex-col gap-3">
        ...
        {nothing ? (
          <p className="text-bone text-[15px] leading-snug">
            {o.wiped ? 'Your shades could not hold their hunting ground and retreated to the lantern. Nothing was lost; nothing was gained. Send them somewhere gentler, or make them stronger.' : 'You rested at the lantern. No shades were hunting for you, so nothing was gathered. Recruit a shade and the ash will work while you sleep.'}
          </p>
        ) : (
          <div className="grid grid-cols-[1fr_auto] gap-x-4 gap-y-1 text-[15px]">
            ...
            <span className="text-bone/70">Marrow</span><span className="font-num text-ember-hot text-right">{fmt(Number(o.marrow))}</span>
            ...
            {o.phantomXp !== '0' && <><span className="text-bone/70">Shade experience</span><span className="font-num text-parchment text-right">{fmt(Number(o.phantomXp))} each</span></>}
          </div>
        )}
        <p className="text-[13px] text-bone/70">Offline never costs you anything: no deaths, no lost marrow, no dropped remains. Your Tallowdraught is full.</p>

  ...
  for (let seed = 1; seed <= 12; seed++) {
    it(`seed ${seed}`, () => {
      ...
      for (let i = 0; i < 4000; i++) {
        ...
        else if (roll < 0.6) actions.push({ type: 'estus' });
        ...
        else if (roll < 0.8) actions.push({ type: 'travel', zone: 'approach', tier: randInt(r, -2, 4) });
        else if (roll < 0.82) actions.push({ type: 'retreat' });
        else if (roll < 0.83) actions.push({ type: 'abandonBloodstain' });
        else if (roll < 0.84) actions.push({ type: 'chooseBossSoul', boss: 'coldPyreWarden', choice: rand(r) < 0.5 ? 'weapon' : 'spell' });
        else if (roll < 0.85) actions.push({ type: 'cast', slot: 0 });
        // occasionally gift souls/materials so purchases actually happen
        if (rand(r) < 0.02) s.souls = s.souls.add(randInt(r, 0, 5000));

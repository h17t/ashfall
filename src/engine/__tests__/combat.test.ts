...
describe('damage formula', () => {
  it('scales with the weapon grade and stat', () => {
    ...
    s.player.stats.str = 40;
    const strong = weaponDamage(s, mods).total;
    expect(strong.gt(base)).toBe(true);
    // D grade at 40 points: 0.5 * 0.95 = +47.5% from str alone (dex still 10 -> 0.15)
  ...
  it('never produces NaN or negative damage', () => {
    const s = newGame(1);
    const mods = computeMods(s);
    for (const k of ['vig', 'end', 'str', 'dex', 'int', 'fth'] as const) s.player.stats[k] = 0;
    const br = weaponDamage(s, mods);
...
describe('clicking and stamina', () => {
  ...
  it('exhausted attacks deal reduced damage and build no stagger', () => {
    const s = withEnemy();
    s.player.stamina = 0;
    ...
    expect(s.encounter.enemy!.stagger).toBe(0);
  });
...
describe('stagger and riposte', () => {
  it('fills the stagger meter, opens a riposte window, and multiplies damage', () => {
    const s = withEnemy();
    const e = s.encounter.enemy!;
    e.hp = D(1e9); e.hpMax = D(1e9);
    e.poise = 5; // one hit staggers
    const ev = advance(s, 0, [{ type: 'click' }]);
    expect(ev.some((x) => x.type === 'stagger')).toBe(true);
    expect(e.riposte).toBeGreaterThan(0);
    const mods = computeMods(s);
    ...
    expect(hit.riposte).toBe(true);
    // ±8% variance and possible crit; riposte is 3x for the starting sword
    expect(hit.dmg.toNumber()).toBeGreaterThan(normal.total.toNumber() * getWeapon(s.player.weapon).riposteMult * 0.9);
  });
  it('riposte window expires and reports a miss when unused', () => {
    const s = withEnemy();
    const e = s.encounter.enemy!;
    e.hp = D(1e9); e.hpMax = D(1e9);
    e.poise = 5;
...
describe('enemy attacks and dodging', () => {
  ...
  it('a dodge inside the perfect window avoids damage and grants a buff', () => {
    ...
    const e = s.encounter.enemy!;
    e.hp = D(1e9);
    // force a telegraph about to land
...
describe('estus', () => {
  it('heals a fraction of max hp and consumes a flask', () => {
    const s = withEnemy();
    s.player.hp = 10;
    const ev = advance(s, 0, [{ type: 'estus' }]);
    expect(s.player.estus).toBe(BALANCE.player.estusStart - 1);
    expect(s.player.hp).toBeGreaterThanOrEqual(10 + Math.floor(s.player.hpMax * BALANCE.player.estusPotency) - 1);
    expect(ev.some((x) => x.type === 'heal')).toBe(true);
  });
  it('resting refills estus and hp without losing souls', () => {
    const s = withEnemy();
    s.player.hp = 5; s.player.estus = 0; s.souls = D(500);

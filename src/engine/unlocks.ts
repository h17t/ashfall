...
export function checkUnlocks(state: GameState, mods: Mods, events: GameEvent[]) {
  ...
  if (!f.autoAttack && (first.cleared >= 1 || state.prestige.kindles > 0 || state.stats.playTime >= 360)) {
    ...
    events.push({ type: 'unlock', what: 'autoAttack', text: 'Hollow Instinct: your hands remember the swing. Auto-attack unlocked. Clicking still adds on top.' });
  }
  if (!f.firstDeath && state.stats.deaths > 0) {
    f.firstDeath = true;
  }
  if (!f.infusionUnlocked && (state.materials.coal ?? 0) > 0) {
    f.infusionUnlocked = true;
    events.push({ type: 'unlock', what: 'infusion', text: 'Cinder Coal: the bonfire can now infuse weapons.' });
  }
}

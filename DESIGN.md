...
- **Every new thing has words.** Affixes gained a line of lore each (shown on the reforge sheet), joining the boons, the Arts, the sets, the afflictions and the hours, all validated for length by `validateContent()`; the banned-terms linter caught three borrowed words in the first draft of the Arts and the afflictions and they were renamed before they shipped.

## Pass 3: test and polish

- **Every audit fails the build.** `npm run ci` and the workflow run, in order: typecheck; the tests (engine, migrations for every save version with a fixture each, fuzzed corruption, pacing, the asset audit, banned terms); the banned-terms linter; the hover audit; the build; the payload budget (gzipped shell, precache size, time to a landed strike on slow 4G at 4x CPU); PWA completeness (`tools/audit/manifest.mjs`); then against the built game at a phone viewport: touch targets, accessibility and contrast (`tools/audit/a11y.mjs`: every control named, every image with alt, every dialog labelled, focus visible, reduced motion honoured, text contrast where the surface is a flat colour), the device matrix (`tools/audit/devices.mjs`: iPhone SE, iPhone 14, Pixel 7, Galaxy S22, iPad, desktop, each running load, strike, level, settings, back to combat, with a screenshot into `art/devices/`), and the interruption tests.
- **Reading.** Two settings under Reading: Plain type swaps every face for a humanist sans with wider letter, word and line spacing (Atkinson Hyperlegible where the system has it, Verdana or the system face otherwise); Colour-safe statuses tell bleed, poison and frost apart by shape (square, diamond, ring) and by text, and move the poison gauge and mark off green against red.
- **Onboarding by playing.** The hints know the hand: on a touch device they say tap Strike, tap Dodge, open Lantern in the bar below; on a keyboard they keep the keys. No text wall was added; the first minute teaches with the hint card and the Strike button.
- **Found by the matrix.** The Lantern's eight sub-tabs overflowed a phone's width with Lore and Settings cut off; sub-tab rows wrap now. The level buttons were a bare "+" to a screen reader; they say which stat. An iPad held upright (820px) was tried at the two-column layout and the fight column collapsed to a third of the width, so upright tablets keep the phone shape, wider, and turn sideways for two columns.
- **Payload.** The section panels load behind the combat shell through `src/ui/shell/loadable.tsx` (no Suspense, see round 13); the shell is 181 KB gzipped and a strike lands in 2.7 s on the budget audit's throttled phone.
- **QA.md** carries what no audit judges: the feel of the Reprisal, the haptics, whether the Black Hour is exciting.

## Pass 3: after the first phone

- **Plates under a sub-path.** GitHub Pages serves the game under the repository's name, and the manifest built every plate path from the origin root (`/art/...`), so the deployed game had no pictures. `assets/manifest.ts` now builds paths from Vite's `BASE_URL` (`./art/...` in the bundle; the plain root for the Node tools that write the files). `tools/audit/deploy.mjs` serves `dist/` under a sub-path with the service worker on and walks every screen: any request that fails, any image that never decodes, a worker that does not install, fails the build. A relative path resolves against the page, so the game can be published under any path without a rebuild.
- **Reachability is measured, not assumed.** `tools/audit/reach.mjs` opens the game on twenty viewports (phones upright and sideways, tablets both ways, laptop widths from 900 to 1440) and, on every pillar and sub-tab, scrolls each control's own scroller to it and checks that its centre is inside the screen and that a tap there lands on it. The first run found 2,377 problems; the fixes: sub-tab rows wrap in every layout (sideways they scrolled with no scrollbar and no way to reach Settings); the wide layout needs 560px of height, so a sideways phone (915×412) gets the landscape shape instead of a rail that left the arena a third of the screen; sideways and wide, the section column and the combat column are each their own scroller (the page never scrolls), the arena gives way before the action bar (`clamp(200px, 100dvh − 400px, 560px)`), orders and log scroll in a box under the action bar, and the hint sits in the combat column instead of floating over the section; in portrait Combat the column grows with its content so the nav padding lands after the last order rather than under it; the Vestige tree closes its columns to fit a narrow section.
- **A play-through in CI.** `tools/audit/playthrough.mjs` plays the first cycle through the real controls on a phone held upright and held sideways: strike by hand, wait for the Instinct and write a Standing Order through the chip sheet, level at the Lantern, buy and wield a weapon, call a shade, take the Stair (descend, the boon sheet, a floor, withdraw, the haul), reforge, swear to a creed, change quality and Plain type, snuff the flame and sit through the cinema, reload. Time passes through the engine between taps; the grind that would take an hour is skipped by editing the save. The script stops for what the game stops for (a death cinema, the floor-cleared sheet that waits for a choice), which is how it found that a cinema card carried no accessible name.

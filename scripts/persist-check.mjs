...
await page.evaluate(() => { const g = window.__ashfall.getState(); g.state.souls = g.state.souls.add(4242); g.state.materials.shard = 7; });
await page.waitForTimeout(11000); // autosave
const before = await page.evaluate(() => window.__ashfall.getState().state.souls.toString());
await page.reload({ waitUntil: 'networkidle' });
await page.waitForTimeout(500);
const after = await page.evaluate(() => ({ souls: window.__ashfall.getState().state.souls.toString(), shard: window.__ashfall.getState().state.materials.shard }));
...
await ctx.addInitScript(() => {
  ...
  const raw = localStorage.getItem('ashfall.save'); if (!raw) return;
  ...
  localStorage.setItem('ashfall.save', JSON.stringify(blob));
...
await ctx.addInitScript(() => {
  ...
  const raw = localStorage.getItem('ashfall.save'); if (!raw) return;
  localStorage.setItem('ashfall.save', raw.slice(0, 200));
  localStorage.setItem('__corrupted', '1');
});
await page.reload({ waitUntil: 'networkidle' });
await page.waitForTimeout(600);
console.log('corruption banner shown:', (await page.getByText(/could not be loaded/).count()) > 0, 'souls from backup:', await page.evaluate(() => window.__ashfall.getState().state.souls.toString()));
await browser.close();

...
- **Pass 3 (rename, mobile-first, deep content) in progress.** The name is MOURNWAKE (NAMING.md); the fiction is LORE.md; the banned-terms linter runs in `npm test`.
  ...
  - [x] The forward path without menus: push-on buttons in the arena, a tappable goal line, reward floats, hold-to-strike
  - [x] After the first hand: the fight waits in menus and under sheets, the dodge commits inside a 0.9 s window with feedback, a goal line on every screen and an opening sheet
  - [x] After the first phone: plates resolve under the Pages sub-path (deploy audit), every control reachable on 20 viewports (reach audit), a real play-through on both orientations in CI, a seeded monkey (which found the sheet that would not close)

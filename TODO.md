...
- **Pass 3 (rename, mobile-first, deep content) in progress.** The name is MOURNWAKE (NAMING.md); the fiction is LORE.md; the banned-terms linter runs in `npm test`.
  ...
  - [x] After the first phone: plates resolve under the Pages sub-path (deploy audit), every control reachable on 20 viewports (reach audit), a real play-through on both orientations in CI
